...
import Loading from "./loading/topten";

export default function TopTen({ projects, title, className, top = false }: any) {
  ...
  return (
    <div className={className}>
      <div className="w-full mx-auto">
        {!top && <h3 className="body-24-bold text-blue flex items-center xl:w-[1146px] mx-auto mb-4">{title}</h3>}
        {top && title}
        <div className="relative group translate-y-[-10px] overflow-hidden">
          {projectRender?.length > 3 ? (
            <div>
              ...
              <div
                className="hidden lg:block absolute left-0 opacity-0 group-hover:opacity-100 transition duration-200 ease-in-out"
                style={{
                  top: "50%",
              ...
              <div
                className="hidden lg:block absolute right-0 opacity-0 group-hover:opacity-100 transition duration-200 ease-in-out"
                style={{
                  top: "50%",
                  transform: "translateY(-50%)",
                }}
            ...
            <div className="md:flex xl:w-[1146px] mx-auto md:translate-x-[-15px]">
              {projectRender?.map((item: any, index: number) => {
                return (
                  <div className=" md:px-[15px] py-5 md:py-7" key={item.image + index}>
                    <Project
                      data={item}
